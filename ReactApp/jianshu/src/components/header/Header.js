...
class Header extends Component {
        ...
        <SearchInfo
          ...
          <SearchInfoTitle>
            ...
            <SearchInfoSwitch onClick={handlerChangePage.bind(this, page, newSearchList.length)}>
              ...
              <i ref={(i)=>{this.icon = i}} className="anticon anticon-sync spin">
  ...
  render() {
    const { focused, handleInputFocus, handleInputBlur, searchList, login } = this.props; //解构赋值后的函数，使用箭头函数的方式调用，后让this指向丢失.
    return (
      <div>
        <HeaderWrapper>
          <Link to='/'>
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handlerChangePage(currentPage, totalPage) {
      /**通过代码实现旋转动画效果 */
      const spin = this.icon;//通过ref获取DOM元素
